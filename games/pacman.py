...
class PacmanGame:
    def __init__(self, screen):
        ...
        self.maze_width = 19
        self.maze_height = 21
        self.cell_size = min(self.width // self.maze_width, self.height // self.maze_height)
        
        # Позиция лабиринта на экране
        self.maze_x = (self.width - self.maze_width * self.cell_size) // 2
        self.maze_y = (self.height - self.maze_height * self.cell_size) // 2
        ...
        self.ghost_colors = [(255, 0, 0), (255, 0, 255), (0, 255, 255), (255, 165, 0)]
        ...
        self.pacman_x = 9
        self.pacman_y = 15
        self.pacman_direction = 0  # 0=вправо, 1=вниз, 2=влево, 3=вверх
        self.pacman_next_direction = 0
        self.pacman_moving = False  # Движется ли Pac-Man
        self.eating_animation = 0.0  # Анимация поедания точки
    ...
    def generate_maze(self):
        ...
        for y in range(1, self.maze_height - 1, 2):
            for x in range(1, self.maze_width - 1, 2):
                ...
                if random.random() < 0.7:  # 70% шанс горизонтального прохода
                    if x + 1 < self.maze_width - 1:
                        maze[y][x + 1] = 2
                
                if random.random() < 0.7:  # 70% шанс вертикального прохода
                    if y + 1 < self.maze_height - 1:
                        maze[y + 1][x] = 2
    ...
    def update_pacman(self, dt):
        ...
        if self.pacman_moving:
            ...
            if self.pacman_step_timer >= 0.2:  # Шаг каждые 0.2 секунды
                ...
                next_x, next_y = self.get_next_position(self.pacman_x, self.pacman_y, self.pacman_next_direction)
                if self.is_valid_position(next_x, next_y):
                    self.pacman_direction = self.pacman_next_direction
                
                # Движение в текущем направлении
                next_x, next_y = self.get_next_position(self.pacman_x, self.pacman_y, self.pacman_direction)
                if self.is_valid_position(next_x, next_y):
    ...
    def update_ghosts(self, dt):
        ...
        if self.ghost_move_timer >= 0.3:  # Движение каждые 0.3 секунды
            ...
            for ghost in self.ghosts:
                ...
                # Движение
                next_x, next_y = self.get_next_position(ghost['x'], ghost['y'], ghost['direction'])
                if self.is_valid_position(next_x, next_y):
                    ghost['x'] = next_x
                    ghost['y'] = next_y
                else:
    ...
    def draw_pacman(self):
        ...
        if self.eating_animation > 0:
            # Анимация поедания - рот закрывается
            mouth_open = 0.05  # Рот закрыт при поедании
        else:
            # Рот всегда одинакового размера (не анимируется при движении)
            mouth_open = 0.05  # Рот почти закрыт
        ...
        if self.eating_animation > 0:
            ...
            animated_mouth_angle = mouth_angle * (1.0 - animation_factor * 0.8)  # От mouth_angle до 0.2 * mouth_angle
            ...
            for i in range(num_points + 1):
                angle = animated_start_angle + (animated_end_angle - animated_start_angle) * i / num_points
        ...
        if self.eating_animation <= 0.9:  # Глаз виден когда рот не полностью закрыт
            ...
            pygame.draw.circle(self.screen, (255, 255, 255), (int(eye_x), int(eye_y)), eye_radius + 1)
    ...
    def draw_ghosts(self):
        ...
        for ghost in self.ghosts:
            ...
            pygame.draw.circle(self.screen, ghost['color'], 
                             (center_x, center_y - ghost_height // 4), ghost_width // 2)
            
            # Нижняя часть - прямоугольник с волнистым низом
            ghost_rect = pygame.Rect(center_x - ghost_width // 2, 
                                   center_y - ghost_height // 4, 
                                   ghost_width, ghost_height // 2)
            ...
            # Левый глаз
            pygame.draw.circle(self.screen, (255, 255, 255), 
                             (center_x - ghost_width // 4, eye_y), eye_size)
            pygame.draw.circle(self.screen, (0, 0, 0), 
                             (center_x - ghost_width // 4, eye_y), eye_size // 2)
