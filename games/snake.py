...
class SnakeGame:
    def __init__(self, screen):
        ...
        self.bomb_timer = 0.0
        self.bomb_redness = 0.0  # Уровень покраснения бомбы (0-1)
        self.bomb_target = random.randint(4, 6)  # Случайный интервал 4-6 яблок
        
        # Инициализация переменных золотых яблок
        self.golden_apple = None
        self.apples_since_golden = 0
        self.golden_apple_timer = 0.0
        self.golden_apple_target = random.randint(4, 6)  # Случайный интервал 4-6 яблок
        self.score_animations = []  # Анимации +10 очков
        
        # Инициализация переменных анимации взрыва
        self.explosion_animations = []  # Анимации взрывов бомб
    ...
    def reset_game(self):
        ...
        self.paused = False
        self.move_timer = 0
        self.base_speed = 0.15  # Базовая скорость
        self.current_speed = self.base_speed
        self.level_up_timer = 0  # Таймер для показа уведомления о повышении уровня
    ...
    def generate_golden_apple(self):
    ...
    def create_explosion_animation(self, bomb_pos):
    ...
    def draw(self):
        ...
        if not self.game_over:
            ...
            for i, segment in enumerate(self.snake):
                ...
                if i == 0:
                    ...
                    pygame.draw.rect(self.screen, (255, 255, 255), 
                                   (x, y, self.grid_size, self.grid_size), 2)
                else:
                    # Тело змейки
                    pygame.draw.rect(self.screen, self.snake_color, 
            ...
            pygame.draw.rect(self.screen, (255, 255, 255), 
                           (food_x, food_y, self.grid_size, self.grid_size), 2)
            ...
            if self.bomb:
                ...
                pygame.draw.circle(self.screen, bomb_color, 
                                 (center_x, center_y), pulse_radius)
                ...
                pygame.draw.circle(self.screen, border_color, 
                                 (center_x, center_y), pulse_radius, 2)
                ...
                sparkle_colors = [
                    (min(255, int(255 * sparkle_intensity)), min(255, int(255 * sparkle_intensity)), 0),
                    (min(255, int(255 * sparkle_intensity)), min(255, int(200 * sparkle_intensity)), 0),
                    (min(255, int(255 * sparkle_intensity)), min(255, int(150 * sparkle_intensity)), 0)
                ]
                for i in range(3):
                    sparkle_x = fuse_x + i * 2
                    sparkle_y = fuse_y - i * 2
                    pygame.draw.circle(self.screen, sparkle_colors[i % 3], 
            ...
            for animation in self.explosion_animations:
                if animation['alpha'] > 0:
                    ...
                    particle_surface = pygame.Surface((animation['size'] * 2, animation['size'] * 2), pygame.SRCALPHA)
                    ...
                    pygame.draw.circle(particle_surface, particle_color, 
                                     (animation['size'], animation['size']), animation['size'])
                    particle_surface.set_alpha(animation['alpha'])
                    
                    # Позиционируем частицу
                    particle_rect = particle_surface.get_rect(center=(animation['x'], animation['y']))
    ...
    def draw_ui(self):
        ...
        if self.level_up_timer > 0:
            ...
            level_up_rect = level_up_surface.get_rect(center=(self.width // 2, self.height // 2 - 100))
