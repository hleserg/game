...
class TetrisGame:
    ...
    def reset_game(self):
        ...
        self.fall_time = 0
        self.fall_speed = 0.5
        self.fast_fall_speed = 0.05  # Быстрое падение при зажатии вниз
            ...
            if event.type == pygame.KEYDOWN:
                if self.game_over:
                    ...
                    if event.key == pygame.K_DOWN:
                        ...
                        new_piece['rotation'] = (new_piece['rotation'] + 1) % len(new_piece['shape'])
                        if self.is_valid_position(new_piece, self.piece_x, self.piece_y):
                            self.current_piece = new_piece
                    elif event.key == pygame.K_SPACE:
                        # Быстрое падение
                        while self.is_valid_position(self.current_piece, 
                                                   self.piece_x, self.piece_y + 1):
                            self.piece_y += 1
    ...
    def update(self, dt):
        ...
        if self.move_timer >= move_speed:
            ...
            if fast_left and self.is_valid_position(self.current_piece, 
                                                  self.piece_x - 1, self.piece_y):
                self.piece_x -= 1
            elif fast_right and self.is_valid_position(self.current_piece, 
                                                     self.piece_x + 1, self.piece_y):
                self.piece_x += 1
