...
class ArkanoidGame:
    ...
    def reset_game(self):
        ...
        self.ball_speed_y = -5
        self.ball_speed = 5
        self.max_ball_speed = 8  # Максимальная скорость мяча
        
        # Система прогрессивного увеличения скорости
        self.target_ball_speed = 5  # Установленная скорость мяча
        self.speed_increase_timer = 0.0  # Таймер для увеличения скорости
        self.speed_increase_interval = 20.0  # Интервал увеличения скорости (20 секунд)
    ...
    def update(self, dt):
        ...
        for i, rocket in enumerate(self.rockets):
            ...
            rocket_rect = pygame.Rect(rocket['x'], rocket['y'], self.rocket_width, self.rocket_height)
        ...
        extra_balls_to_remove = []
        for i, ball in enumerate(self.extra_balls):
            # Движение мяча
            ball['x'] += ball['speed_x']
            ball['y'] += ball['speed_y']
            ...
            if ball['speed_x'] != 0:
                ...
                ball_rect_x = pygame.Rect(future_x - ball['radius'], ball['y'] - ball['radius'], 
                                         ball['radius'] * 2, ball['radius'] * 2)
                ...
                for block in self.blocks:
                    if not block['destroyed'] and block['rect'].colliderect(ball_rect_x):
                        ...
                        if random.randint(1, 10) == 1:
                            for j in range(3):
                                angle = (j * 120) + random.randint(-30, 30)  # Разлет в разные стороны
            ...
            if ball['speed_y'] != 0:
                ...
                ball_rect_y = pygame.Rect(ball['x'] - ball['radius'], future_y - ball['radius'], 
                                         ball['radius'] * 2, ball['radius'] * 2)
            ...
            if paddle_rect.colliderect(ball_rect) and ball['speed_y'] > 0:
                ...
                angle = max(-0.87, min(0.87, angle))  # Ограничиваем угол до ±100 градусов (sin(100°) ≈ 0.87)
        ...
        all_extra_balls_fell = len(self.extra_balls) == 0 or all(ball['y'] > self.height for ball in self.extra_balls)
        ...
        if paddle_rect.colliderect(ball_rect) and self.ball_speed_y > 0:
            ...
            self.ball_speed_x = angle * self.ball_speed
    ...
    def check_block_collisions(self):
        ...
        if self.ball_speed_x != 0:
            ...
            ball_rect_x = pygame.Rect(future_x - self.ball_radius, self.ball_y - self.ball_radius, 
                                     self.ball_radius * 2, self.ball_radius * 2)
        ...
        if self.ball_speed_y != 0:
            ...
            ball_rect_y = pygame.Rect(self.ball_x - self.ball_radius, future_y - self.ball_radius, 
                                     self.ball_radius * 2, self.ball_radius * 2)
    ...
    def next_level(self):
        ...
        self.target_ball_speed = min(self.target_ball_speed + 0.5, self.max_ball_speed)  # Увеличиваем установленную скорость
        self.create_blocks()
    ...
    def draw(self):
        ...
        if not self.game_over:
            ...
            pygame.draw.circle(self.screen, (200, 200, 200), 
                             (int(self.ball_x), int(self.ball_y)), self.ball_radius, 2)
    ...
    def draw_ui(self):
        ...
        if self.game_over:
            ...
            complete_rect = complete_surface.get_rect(center=(self.width // 2, self.height // 2 - 50))
